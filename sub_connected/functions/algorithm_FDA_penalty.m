...
% Optimization
penalty_factor = 0.1; % initial value of the penalty factor
...
function [A, t] = update_analog_beamformer(para, W, D, t)
    t_search = 0:para.t_max/1e3:para.t_max;

    Phi = zeros(para.N_sub, para.N_RF, para.M);
    for n = 1:para.N_RF
        for m = 1:para.M
            p_mn = D(:,:,m)'; p_mn = p_mn(:,n);
            Phi(:,n,m) = W((n-1)*para.N_sub+1:n*para.N_sub,:,m)*p_mn;
        end
    end
    
    
    % update PS coefficients
    N_sub_sub = para.N_sub/para.N_T; % the number of antennas connected to each TTD of one RF chain
    A_PS = zeros(para.N_sub, para.N_RF);
    for n = 1:para.N_RF
        for q = 1:para.N_T
            p_nq = Phi((q-1)*N_sub_sub+1:q*N_sub_sub, n, :);  
            p_nq = squeeze(p_nq);
            t_nq = t(q,n);
            a_nq = sum(p_nq*diag(exp(1i*2*pi*para.fm_all*t_nq)), 2);
            A_PS((q-1)*N_sub_sub+1:q*N_sub_sub, n) = a_nq./abs(a_nq);
        end
    end
    ...
    for n = 1:para.N_RF
        for q = 1:para.N_T
            p_nq = Phi((q-1)*N_sub_sub+1:q*N_sub_sub, n, :);  
            p_nq = squeeze(p_nq);
            a_nq = A_PS((q-1)*N_sub_sub+1:q*N_sub_sub, n);
            psi_nq = p_nq'*a_nq;
...
%% Calculate the overall TTD-based analog beamformer at frequency f
