function [R, A, D, t] = algorithm_HTS_PNF(para, H, user_r, user_theta, W_initial)
%The piecewise-near-field (PNF)-based heuristic two-stage approach
%  [R, A, D, t] = algorithm_HTS_PNF(para, H, user_r, user_theta, W_initial)
%Inputs:
...
for n = 1:para.N_RF
    ...
    % initialize TTD coeffocients
    t_n = t_n - min(t_n);
    t_n(t_n>para.t_max) = para.t_max;

    % iterative optimization of TTD coefficients
    obj_value_max_pre = 0;
    iter_max = 40; % maximum iteration number
    for step = 1:iter_max
        for l = 1:para.N_T
            t_n_null = t_n; t_n_null(l) = []; % remove the l-th entry
